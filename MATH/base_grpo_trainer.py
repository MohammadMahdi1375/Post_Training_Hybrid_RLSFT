...
ADAPTOR_NAME = os.environ.get("ADAPTOR_NAME", "/home/mohammad-m/TTT/saved_model/MATH/llama_sft_E_3_512_5e-5/")
DATA_FILE    = os.environ.get("TRAIN_FILE", "/home/mohammad-m/TTT/Post_Training_Hybrid_RLSFT/MATH/data/train_data.json")
# DATA_FILE    = os.environ.get("TRAIN_FILE", "/home/mohammad-m/TTT/Post_Training_Hybrid_RLSFT/MATH/data/train_data_hard.json")
OUT_DIR      = os.environ.get("OUTPUT_DIR", "../saved_model/MATH/llama_mixed_1_512_1e-55")
MERGED_DIR   = os.environ.get("MERGED_DIR", "../saved_model/MATH/llama_mixed_merged_1_512_1e-55")
...
TEMPERATURE           = int(os.environ.get("TEMPERATURE", 1.0))
...
if not USE_LORA:
    model = PeftModel.from_pretrained(model, ADAPTOR_NAME)
...
def reasoning_score(pre_box: str) -> float:
    ...
    if len(toks) >= 3*24:  len_bonus += 0.3
    if len(toks) >= 3*48:  len_bonus += 0.4
    if len(toks) >= 3*96:  len_bonus += 0.3
...
print(f"Finetuned model: {OUT_DIR}")
